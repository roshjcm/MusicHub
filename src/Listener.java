...
public class Listener {
	...
	public boolean checkOutCart(int order_id) {
		...
		try {
			...
			ArrayList<Integer> art_id= new ArrayList<Integer>();
			...
			while (rs.next()) { 
				...
				earned.add(M);
				art_id.add(A);
				stmt.executeUpdate("INSERT INTO moneyEarned(artist_id,order_id,money_received) "
						+ "values(" + A + "," + order_id + ","+ M + ");");
			...
			for(int i = 0; i<art_id.size() ; i++) {
				Artist artist = new Artist(con, "", "", "", "", "", "","", "");
				artist.logIn(art_id.get(i));
				artist.depositEarnings(earned.get(i));
			}
